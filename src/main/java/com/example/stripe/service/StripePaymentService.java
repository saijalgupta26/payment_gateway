...
import com.example.stripe.dto.*;
import com.example.stripe.exception.CustomerNotFoundException;
import com.example.stripe.exception.PaymentMethodNotFound;
import com.example.stripe.exception.PaymentNotConnect;
import com.example.stripe.repository.PaymentIdRepository;
import com.example.stripe.repository.PaymentMethodRepository;
import com.example.stripe.repository.StripeRepository;
...
public class StripePaymentService {
    ...
    @Value("${stripe.apikey}")
    String stripekey;
    public String customerCreate(CustomerDetails customerDetails) throws StripeException {
    ...
    public String paymentWithCustomer(String customerId,String paymentMethodId,Payment payment) throws StripeException {
        payment = new Payment();
        Stripe.apiKey=stripekey;
        Optional<Payment> payment1=paymentIdRepository.findByCustomerIdAndPaymentId(customerId,paymentMethodId);
        if(!payment1.isEmpty())
        {
            return payment1.get().getPaymentId();
    ...
    public String customerAlreadyExists(CustomerDetails customerDetails) throws CustomerNotFoundException {
        Optional<CustomerDetails> customer = stripeRepository.findByemail(customerDetails.getEmail());
